...
def write_final_product_report(
    *,
    ...
    lines.append("")
    lines.append("- `Tier (publication)` is the primary claim status.")
    lines.append("- `q (strict)` is the multiplicity-adjusted screening statistic; lower is stronger.")
    lines.append("- Congress unified-vs-divided rows are robustness diagnostics for confounding, not causal attribution.")
    lines.append("- If counts are mostly exploratory, report direction and magnitude as descriptive rather than claim-grade.")
    lines.append("")
    ...
    lines.append(".venv/bin/rb publication-bundle --profile strict_vs_baseline")
    lines.append("```")
